...
const normalizePhone = (phone) => {
  ...
  if (raw.startsWith("+62"))
    ...
    normalized = `+62${raw.slice(1)}`;

  else if (raw.startsWith("8"))
    normalized = `+62${raw}`;

  return /^\+62\d{8,13}$/.test(
    normalized
  )
    ? normalized
    : null;
...
exports.register =
  async (req, res) => {
    ...
    try {
      ...
      if (
        ...
        return res.status(400)
          .json({
            success:false,
            message:
              "Name, email, phone, dan password wajib diisi."
          });
      }
      ...
      if (!normalizedPhone) {
        ...
        return res.status(400)
          .json({
            success:false,
            message:
              "Format nomor HP harus menggunakan +62."
          });
      }
      ...
      console.log(
        `[OTP REGISTER] ${normalizedPhone} -> ${otpCode}`
      );
      ...
      return res.status(201)
        .json({
          ...
          message:
            "Registrasi berhasil. OTP telah dikirim via WhatsApp.",
      ...
      return res.status(500)
        .json({
          ...
          message:
            "Terjadi kesalahan server saat registrasi."
        });
    }
};
  ...
  try {
    ...
    if (!identity) {
      return res.status(400).json({
        success: false,
        message: "Identity (email/phone) wajib diisi."
      });
    }
    ...
    return res.status(200).json({
      ...
      message:
        "OTP baru berhasil dikirim via WhatsApp. Jangan Buang, Jadikan Berkah."
    });
    ...
    return res.status(500).json({
      ...
      message:
        "Terjadi kesalahan server saat mengirim ulang OTP."
    });
  }
};

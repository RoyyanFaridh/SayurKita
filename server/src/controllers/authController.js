...
exports.verifyOTP = async (req, res) => {
  try {
    ...
    if (!identity || !otpCode) {
      return res.status(400).json({
        success: false,
        message: "Identity (email/phone) dan OTP wajib diisi.",
      });
    }
    ...
    if (!user.otpCode || !user.otpExpires) {
      return res.status(400).json({
        success: false,
        message: "OTP tidak tersedia. Silakan kirim ulang OTP.",
      });
    }
    ...
    return res.status(200).json({
      success: true,
      message: "Verifikasi berhasil. Jangan Buang, Jadikan Berkah.",
    });
  } catch (error) {
...
exports.login = async (req, res) => {
  try {
    ...
    if (!identity || !password) {
      return res.status(400).json({
        success: false,
        message: "Identity (email/phone) dan password wajib diisi.",
      });
    }
...
exports.resendOTP = async (req, res) => {
  try {
    ...
    return res.status(500).json({
      success: false,
      message: "Terjadi kesalahan server saat mengirim ulang OTP.",
    });
  }
};

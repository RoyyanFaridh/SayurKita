...
export default function StepIndicator({ steps }) {
  return (
    <div className="flex items-start w-full">
      {steps.map((s, i) => (
        <div key={s.num} className="flex flex-col items-center gap-1.5 flex-1 relative min-w-0">
          <div
            ...
            style={
              s.done
                ? { background: 'var(--accent-primary)', borderColor: 'var(--accent-primary)', color: 'var(--color-primary-900)' }
                : s.active
                ? { background: 'var(--color-primary-900)', borderColor: 'var(--color-primary-900)', color: '#ffffff' }

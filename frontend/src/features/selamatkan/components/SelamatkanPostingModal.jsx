...
export default function SelamatkanPostingModal({ onClose, onSuccess }) {
  ...
  const handleLocate = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      pos => {
        ...
        setErrors(p => ({ ...p, lokasi: undefined }));
        setIsLocating(false);
      },
      err => {
  ...
  const validate = () => {
    ...
    if (!lokasi.trim())      errs.lokasi   = 'Lokasi wajib diisi';
    if (lat === null || lng === null) errs.lokasi = 'Klik "Gunakan lokasimu" untuk mendapatkan koordinat';
    return errs;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        showToast(data.message || 'Gagal memposting surplus');
      }
    } catch (err) {
      console.error(err)
      showToast('Terjadi kesalahan saat memposting. Pastikan koneksi internetmu aktif.');
    } finally {
          ...
          <div className="px-5 py-5 flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <textarea
                ...
                rows={3}
                value={deskripsi}
                onChange={e => { setDeskripsi(e.target.value); setErrors(p => ({ ...p, deskripsi: undefined })); }}
              />
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className={`flex items-center gap-2 bg-(--bg-alt) border rounded-md px-3 focus-within:border-(--border-brand) transition-colors duration-150
                ...
                <input
                  ...
                  value={lokasi}
                  onChange={e => { setLokasi(e.target.value); setErrors(p => ({ ...p, lokasi: undefined })); }}
                />
                <button

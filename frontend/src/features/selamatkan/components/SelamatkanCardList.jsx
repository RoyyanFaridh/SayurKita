...
function FotoPlaceholder({ nama }) {
  ...
  return (
    <div className="w-12 h-12 rounded-xl bg-(--bg-subtle) flex items-center justify-center shrink-0 self-start mt-0.5">
...
function SurplusCard({ item, onDetail }) {
  return (
    <div className="flex gap-3 bg-white border border-(--border-subtle) rounded-2xl p-3 shadow-(--shadow-xs) transition-[box-shadow,border-color] duration-150 hover:shadow-(--shadow-md) hover:border-(--border-default) max-[580px]:flex-col max-[580px]:gap-2">
      ...
      <div className="flex-1 min-w-0 flex flex-col gap-1.5">
        ...
        <p className="text-compact-sm text-(--text-secondary) leading-relaxed line-clamp-1 max-[580px]:line-clamp-2">{item.deskripsi}</p>
        <div className="flex items-center gap-3 flex-wrap">
          {[
            ...
            <span key={i} className="inline-flex items-center gap-1 text-compact-xs text-(--text-muted)">
              <Icon size={11} strokeWidth={2} />{text}
            </span>
          ))}
        </div>
        <div className="flex items-center justify-between gap-2 pt-1.5 border-t border-(--border-subsub)/60">
          <div className="flex items-center gap-1.5">
            <div className="w-5 h-5 rounded-full bg-primary-100 text-primary-700 text-compact-xs font-bold flex items-center justify-center">
              {item.pemilik[0].toUpperCase()}
            </div>
            <span className="text-compact-sm text-(--text-secondary) font-medium">{item.pemilik}</span>
          </div>
          <div className="flex items-center gap-1.5">
            <button className="w-7 h-7 flex items-center justify-center bg-transparent border border-(--border-default) rounded-lg text-(--text-muted) cursor-pointer transition-all duration-150 hover:bg-(--bg-secondary-subtle) hover:border-secondary-300 hover:text-secondary-600" aria-label="Simpan">
              <Bookmark size={13} strokeWidth={1.75} />
            </button>
            <button onClick={() => onDetail?.(item)} className="inline-flex items-center gap-1 px-2.5 py-1 bg-transparent border border-(--border-default) rounded-lg text-compact-xs font-medium text-(--text-secondary) cursor-pointer transition-all duration-150 hover:bg-(--bg-surface-3) hover:border-(--border-strong) hover:text-(--text-primary) max-[580px]:hidden">
              <MessageCircle size={13} strokeWidth={1.75} /> Hubungi
            </button>
            <button onClick={() => onDetail?.(item)} className="inline-flex items-center gap-1 px-2.5 py-1 bg-primary-900 text-white border-0 rounded-lg text-compact-xs font-medium cursor-pointer transition-colors duration-150 hover:bg-primary-700">
...
export default function SelamatkanCardList({ items, onDetail }) {
  if (items.length === 0) {
    return (
      <div className="flex flex-col items-center gap-2 py-12 px-6 text-center bg-white border border-(--border-subtle) rounded-xl">
